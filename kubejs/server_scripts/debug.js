ItemEvents.rightClicked("kubejs:debugger", event=>{
    ...
    if(player.isShiftKeyDown()){
        let bbb = BeeBoxPresets["swamp_box_1"](level, buildPos)
        bbb.buildBox()
        player.tell(`build box center at [${buildPos.x}, ${buildPos.y}, ${buildPos.z}]`)
    }
    else{
        let findPos = findCurrentBoxCenter(level, playerPos, 18, 20)
        if(!findPos){player.tell("§2 no find box center")}
        else{
            let bbb = new BeeBoxBuilder(level, findPos).loadCenterData().addDecoration("dress_top_by_lime_stained_glass").giveNestingOrderItem(player, "forest_box_1")
